...
"Update date":"\u66f4\u65b0\u65e5\u671f"};languageToggle.addEventListener("click",function(){isEnglish=!isEnglish;languageToggle.textContent=isEnglish?"\u4e2d\u6587":"EN";document.querySelectorAll("[data-translate]").forEach(a=>{const b=a.getAttribute("data-translate");a.textContent=isEnglish?b:translations[b]})});audio.addEventListener("play",()=>{setButtonState("btnCntPlay",!1);setButtonState("btnRPSart",!0);setButtonState("btnRPEnd",!1);setButtonState("btnRPAdd",!1);setButtonState("btnReset",!1)});
...
debugMenu.addEventListener("mouseenter",()=>{debugSubMenu.classList.remove("hidden")});debugMenu.addEventListener("mouseleave",a=>{"debugSubMenu"!=a.target.id&&debugSubMenu.classList.add("hidden")});debugSubMenu.addEventListener("mouseleave",()=>{debugSubMenu.classList.add("hidden")});helpMenu.addEventListener("mouseenter",()=>{helpSubMenu.classList.remove("hidden")});helpMenu.addEventListener("mouseleave",a=>{"debugSubMenu"!=a.target.id&&helpSubMenu.classList.add("hidden")});
helpSubMenu.addEventListener("mouseleave",()=>{helpSubMenu.classList.add("hidden")});helpSubMenu.addEventListener("click",()=>{helpSubMenu.classList.add("hidden")});const waveSurfer=WaveSurfer.create({container:document.getElementById("waveSurfer"),waveColor:"rgb(200, 0, 200)",progressColor:"rgb(100, 0, 100)",normalize:!0,media:audio});waveSurfer.on("ready",function(){document.getElementById("placeholderWaveform").style.display="none"});
document.getElementById("fileInput").addEventListener("change",async a=>{if(a=a.target.files[0])await storeAudioFile(a.name,a),updatePlayList()});async function storeData(a,b){try{await db[objectStorName].put({key:a,value:b}),console.log(`\u6210\u529f\u5b58\u50a8\u6570\u636e: ${a}`)}catch(c){console.error("\u5b58\u50a8\u6570\u636e\u5931\u8d25:",c)}}
